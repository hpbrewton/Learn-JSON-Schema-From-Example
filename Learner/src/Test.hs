...
oracleExamples = do 
    responses <- sequence $ replicate 100 $ Wreq.get "http://127.0.0.1:3000"
...
main :: IO ()
main = do 
    contents <- readFile "/Users/hb/go/src/latlib/server/data/ex1/object.json"
    let content = A.decode $ convert contents
    let example = ((fromJSON $ Maybe.fromJust content) :: JSON)
    l <- grown example

    let (_, indexedObjs) = indexJSON 0 M.empty example
    let (_, indexedOracles) = indexOracles 0 M.empty oracleN example
    let (n, indexedLatitce) = indexJSONLatticeReffed 0 M.empty l
    m <- sequence $ M.intersectionWith ($) indexedOracles indexedObjs
    putStrLn $ show indexedLatitce
