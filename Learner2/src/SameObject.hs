...
sameObjectGeneralize defs@(DefinitionSchema top schemae) = do 
    ...
    return $ DefinitionSchema top $ HM.unionWith (\(AnyOf items) (SchemaWithOracle current oracle) -> (SchemaWithOracle (AnyOf (current:items)) oracle)) anyOfs schemae
    where 
        ...
        allLabels = HM.toList $ HM.filter isObjectOrNull schemae
